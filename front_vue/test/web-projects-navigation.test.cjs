...
test('limits login redirects to known application pages and project content paths', () => {
    ...
    assert.equal(normalizeInternalRedirect('/files'), '/files')
    assert.equal(normalizeInternalRedirect('/s/AbCDef0123456789_-AbCDef01234567'), '/s/AbCDef0123456789_-AbCDef01234567')
    ...
    assert.equal(normalizeInternalRedirect('/files/1'), '/')
    assert.equal(normalizeInternalRedirect('/s/'), '/')
    assert.equal(normalizeInternalRedirect('/s/AbC09_-token'), '/')
    assert.equal(normalizeInternalRedirect('/s/token/extra'), '/')
    assert.equal(normalizeInternalRedirect('/s/token%2Fextra'), '/')
