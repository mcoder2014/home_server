<template>
  ...
  <section class="surface-card section-card">
    ...
    <el-table :data="items" v-loading="loading" row-key="id" empty-text="没有符合条件的用户">
      <el-table-column label="用户" min-width="190"><template #default="{row}"><el-button link type="primary" @click="viewUser(row)">{{ row.display_name || row.user_name }}</el-button><div class="muted">{{ row.user_name }}</div><div class="muted">ID {{ row.id }}</div></template></el-table-column>
      ...
      <el-table-column label="操作" width="125" fixed="right"><template #default="{row}"><el-button link type="primary" @click="viewUser(row)">详情</el-button><el-dropdown v-if="row.status !== 'deleted'" trigger="click" @command="action => beginAction(row, action)"><el-button link type="primary">更多⌄</el-button><template #dropdown><el-dropdown-menu>
        ...
        <el-dropdown-item command="logout-all">退出全部会话</el-dropdown-item><el-dropdown-item command="reset-password" :disabled="isSelf(row)">重置密码</el-dropdown-item>
  ...
  <el-drawer v-model="detailVisible" :title="detail?.user?.user_name || '用户详情'" size="min(820px, 96vw)"><el-skeleton v-if="detailLoading" animated :rows="7" /><template v-else-if="detail">
    <el-tabs class="detail-tabs">
      <el-tab-pane label="基本信息"><dl class="meta-list"><dt>用户 ID</dt><dd>{{ detail.user.id }}</dd><dt>用户名</dt><dd>{{ detail.user.user_name }}</dd><dt>显示名称</dt><dd>{{ detail.user.display_name }}</dd><dt>联系邮箱</dt><dd>{{ detail.user.contact_email || '—' }}</dd><dt>联系电话</dt><dd>{{ detail.user.contact_mobile || '—' }}</dd><dt>状态 / 角色</dt><dd>{{ statusText[detail.user.status] }} / {{ detail.user.role === 'admin' ? '管理员' : '普通用户' }}</dd><dt>家庭藏书</dt><dd>{{ detail.user.library_enabled ? '已开通 · 全站共用库存读写' : '未开通' }}</dd><dt>WebDAV</dt><dd>{{ webdavText[detail.user.webdav_permission] }} · 共享根目录</dd><dt>来源</dt><dd>{{ sourceText[detail.user.source] || detail.user.source || '—' }}</dd><dt>邀请人</dt><dd>{{ detail.user.invited_by_user_id || '—' }}</dd><dt>创建管理员</dt><dd>{{ detail.user.created_by_user_id || '—' }}</dd><dt>{{ detail.user.source === 'config_import' ? '导入时间' : '创建时间' }}</dt><dd>{{ dateTime(detail.user.source === 'config_import' ? detail.user.imported_at || detail.user.create_time : detail.user.create_time) }}</dd><dt>最近登录</dt><dd>{{ dateTime(detail.user.last_login_at) }}</dd></dl><p class="muted">联系信息不自动用于登录或密码找回。原密码及哈希均不可查看。</p></el-tab-pane>
  ...
  <AdminConfirm v-model="actionVisible" :title="actionTitles[action] || '撤销邀请码'" :description="actionDescriptions[action] || '撤销后邀请码立即永久失效，不返还生成额度。'" :confirm-name="action === 'delete' ? selected?.user_name : ''" :loading="saving" :server-error="actionError" @confirm="executeAction">
    <p v-if="selected" class="muted">目标：{{ selected.display_name || selected.user_name }}（{{ selected.user_name }} / {{ selected.id }}）</p>
...
export default {
  name: 'AdminUsers', components: {AdminConfirm},
  data() { return {
    items: [], cursor: '', hasMore: false, loading: false, error: '', limit: 20, filters: {query: '', status: '', role: '', library_enabled: '', webdav_permission: ''},
    detail: null, detailVisible: false, detailLoading: false, createVisible: false, creating: false, createError: '', createForm: {user_name: '', generate_password: true, password: ''},
  ...
  methods: {
    ...
    async load(reset) { this.loading = true; this.error = ''; try { const data = await accountsApi.users({...this.filters, limit: this.limit, cursor: reset ? '' : this.cursor}); this.items = reset ? data.items : this.items.concat(data.items); this.cursor = data.next_cursor || ''; this.hasMore = !!data.has_more } catch (error) { this.error = error.message } finally { this.loading = false } },
    async viewUser(row) { this.detailVisible = true; this.detailLoading = true; this.detail = null; try { this.detail = await accountsApi.user(row.id) } catch (error) { this.error = error.message; this.detailVisible = false } finally { this.detailLoading = false } },
    ...
    async executeAction(confirmation) {
      ...
      try {
        ...
        if (this.detailVisible) await this.viewUser(this.selected)
