...
export default {
  ...
  methods: {
    ...
    async prepareRollback(record) {
      ...
      try { const values = configValues(this.schema, record.values); this.validation = await accountsApi.validateConfig(this.namespace, values); if (String(this.validation.revision) !== String(this.current.revision)) { this.conflict = true; this.historyVisible = false; return } this.rollbackTarget = record; this.pendingRequest = null; this.publishError = ''; this.confirmVisible = true }
