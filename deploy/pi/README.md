...
本版本没有新增表或 DDL。旧 `account_policy` 缺少 `min_share_password_length` 与 `share_code_length` 时，仅在读取时补充 8、6，保留原 JSON、校验和及历史版本；新发布配置须包含完整字段。账号密码下限允许设为 4，已有值保持不变。发布配置后，旧程序不识别新增字段，不能直接切回旧后端；应部署兼容新字段的修复版本；管理页面回滚历史配置也会补齐新增字段，不能据此恢复旧程序。旧版本严格校验配置字段，单纯恢复前端不影响后端配置。

前后端须使用同一源码提交构建。更新 `/etc/home_server/locations/web_projects_locations.conf`，沿用现有可信代理变量；同步将两个 TLS 网关模板的 `/p/` location 配置应用到实际站点，关闭该路径访问日志与错误日志，增加 `Referrer-Policy: no-referrer`。分别运行 Nginx 配置检查再 reload。保留旧 release 和配置副本用于未发布新策略前的程序回滚。

验收应覆盖：网页配置下限为 4、3 位口令拒绝、4 位口令成功、提高下限后旧密码仍有效；带码链接正确及错误两种情况、地址栏删除所有 code 参数、保留其他参数与片段、原有 Cookie/所有者场景以及仅自己可见的权限约束。
