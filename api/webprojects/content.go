...
func serveProjectContent(c *gin.Context) {
	...
	// Consume query passwords in an isolated page even for owners or existing grants,
	// before any hosted script or resource can observe the original URL.
	if _, hasCode := c.Request.URL.Query()["code"]; hasCode && c.Request.Method == http.MethodGet && passwordHTMLRequest(c, requested) {
...
func servePasswordUnlockPage(c *gin.Context, projectID int64) {
	...
	html := `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>密码阅读</title><script>const shareURL=new URL(location.href);let shareCode=shareURL.searchParams.get('code');if(shareURL.searchParams.has('code')){shareURL.searchParams.delete('code');history.replaceState(history.state,'',shareURL.pathname+shareURL.search+shareURL.hash)}</script><style>body{margin:0;background:#f3f7f5;color:#173f35;font:16px system-ui,sans-serif}.card{box-sizing:border-box;width:min(92vw,430px);margin:12vh auto;padding:28px;background:#fff;border:1px solid #d8e4df;border-radius:18px;box-shadow:0 14px 40px #173f3514}h1{margin:0 0 8px;font-size:24px}p{color:#55736b}input,button{box-sizing:border-box;width:100%;height:46px;border-radius:10px;font:inherit}input{padding:0 12px;border:1px solid #a9beb7}button{margin-top:12px;border:0;background:#176c55;color:#fff;font-weight:650;cursor:pointer}button:disabled{opacity:.65}.error{min-height:22px;color:#b42318;font-size:14px}</style></head><body><main class="card"><h1>此网页受密码保护</h1><p>输入分享者提供的阅读密码。</p><form id="unlock"><input id="password" type="password" autocomplete="current-password" required autofocus><button id="submit" type="submit">解锁网页</button><p class="error" id="error" role="alert"></p></form></main><script>async function unlock(password){const button=document.getElementById('submit'),error=document.getElementById('error');button.disabled=true;error.textContent='';try{const response=await fetch('` + endpoint + `',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({password})});if(!response.ok)throw new Error(response.status===403?'密码不正确':response.status===429?'尝试过于频繁，请稍后重试':'暂时无法解锁，请稍后重试');location.replace(shareURL.pathname+shareURL.search+shareURL.hash)}catch(failure){error.textContent=failure.message||'暂时无法解锁，请稍后重试';button.disabled=false}}document.getElementById('unlock').addEventListener('submit',function(e){e.preventDefault();unlock(document.getElementById('password').value)});if(shareCode!==null){const password=shareCode;shareCode=null;unlock(password)}</script></body></html>`
	c.Header("Cache-Control", "no-store")
