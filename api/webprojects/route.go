...
func InitRouter() error {
	...
	for _, prefix := range []string{"/api/web-share", "/api/web-projects"} {
		...
		data.AddRoute(http.MethodGet, prefix+"/:id/password", requireModule, read, getProjectPassword)
		data.AddRoute(http.MethodPut, prefix+"/:id/password", requireModule, write, putProjectPassword)
		data.AddRoute(http.MethodPost, prefix+"/:id/unlock", requireModule, unlockProject)
