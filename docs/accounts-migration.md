...
Basic 使用独立的密码失败计数与容量上限，网页登录失败不会连带阻断它；同一账号的 Basic 登录别名和两条 WebDAV 路由共享其自身限速。获准账号可直接使用尚未过期的初始密码连接客户端，不必先完成网站首次改密；该例外不解除网站会话的首次改密要求，过期初始密码仍被拒绝。
...
| `account_policy` | 会话 TTL、临时密码期限、密码最低长度、bcrypt 成本 | 用于新签发会话与新设置密码 |
