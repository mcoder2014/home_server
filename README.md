...
数据库账号模式支持受邀注册、个人资料与改密、邀请记录、用户管理、网页审核和通用动态配置。迁移流程与回滚边界见 [账号迁移和配置运维](docs/accounts-migration.md)，个人中心增量升级与会话描述清理见 [运维说明](docs/account-center-operations.md)，页面与交互见 [前端说明](front_vue/README.md)。

| 能力 | 行为 |
| --- | --- |
| 邀请注册 | 存量用户每个 Asia/Singapore 自然月最多生成3个单次邀请码，固定168小时有效；新用户次月获得资格。关闭注册会永久废弃旧未用码。 |
| 个人中心 | 编辑昵称和联系资料、裁剪上传或移除头像；全站优先显示当前昵称与头像。改密验证当前密码后退出旧会话、停用原应用，联系资料不改变登录别名。 |
| 登录管理 | 查看有效会话的登录 IP、UA、客户端、系统、登录时间、到期及剩余时间；可退出选中或其他全部登录，当前登录与应用凭证保持。默认最多同时 5 个有效会话，管理员在账号策略调整上限；满额拒绝新登录。数量不代表物理设备数或在线人数。 |
| 管理员 | 查看用户有效会话数量及详情，达到 10 个提示；可人工封禁或重置昵称、头像。重置需要当前密码、原因与版本校验并记录审计；封禁/删除阻断会话、应用和 WebDAV，恢复不复活旧凭证。不能操作自己的封禁/删除/撤销管理员。 |
...
可选缓存覆盖书目、库位、已发布版本元数据及应用 Token 快照；权限和撤销信息保持实时检查。托管页面支持浏览器口径的每日/累计 PV、近似 UV，保存最近 90 日每日汇总与长期总量，编辑页可查看 7/30/90 日趋势。

新开关默认关闭。统计开启前需要手动执行增量统计表迁移，并配置独立私有访客密钥；应用不会修改 Redis 配置或自动建表。统计只使用普通 Redis 命令和 pipeline，不需要 Lua/EVAL 权限，也不要求特定淘汰策略。完整口径、启用配置、故障边界和回滚步骤见 [Redis 缓存与访问统计](docs/redis-cache-analytics.md)。

### 密码长度配置与网页快速解锁

管理员在“管理中心 → 通用配置 → 账号策略”中调整密码长度，校验并发布后可查看实际生效版本。

| 配置项 | 可配置范围 | 默认值 | 适用范围 |
| --- | --- | --- | --- |
| 账号密码最少字符数 | 4～64 个字符 | 15 | 注册、改密、管理员创建及重置密码 |
| 网页、说明书与文件分享密码最少字节数 | 4～72 个 UTF-8 字节 | 8 | 网页阅读密码、说明书阅读密码、要求登录的文件分享密码 |
| 公开文件分享码长度 | 4～12 位 | 6 | 新建公开文件分享的随机或自定义 ASCII 字母数字分享码 |

ASCII 字符各占 1 字节，因此三类口令均可配置为 4 位。账号与分享密码最多 72 个 UTF-8 字节；账号自动生成的临时密码仍至少 20 位。配置只约束新建或修改操作，已有密码与分享码仍按原值验证。升级不会覆盖现有账号策略，也不自动降低默认长度。

网页阅读链接支持 `/p/report/?code=1234`。独立解锁页会立即从地址栏删除全部 `code` 参数，再通过原 POST 解锁接口验证密码；成功后打开原页面，保留其他查询参数和 `#` 定位。错误密码显示“密码不正确”，可直接手动重试。原始页面和增强页面均适用，已有解锁 Cookie 或所有者访问也会清除参数。链接不会绕过仅自己、指定用户等访问范围；文件分享和说明书的 URL 不使用此参数。

`code` 仍会出现在首次请求中，地址栏清除仅避免打开后复制时再次带出密码。服务端日志会对该参数脱敏；部署应同步更新网页代理日志配置，避免代理层记录原始请求。分享完整带码链接等于同时分享密码。
